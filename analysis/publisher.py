...
def _get_trusted_publisher_paths() -> List[str]:
    return [
        ...
        os.path.join(_CURRENT_DIR, "signature", "trusted_publishers.json"),
        os.path.join(_CURRENT_DIR, "..", "signature", "trusted_publishers.json"),
...
def _load_trusted_publishers() -> List[str]:
    ...
    # Fallback default trusted list if file cannot be read
    return [
        "Google LLC",
...
def is_trusted_publisher(publisher: Optional[str]) -> bool:
    ...
    for company in trusted_list:
        ...
        # Word boundary search to prevent spoofing (e.g. "Fake Google LLC")
